...
describe RestPack::Serializer::SideLoading do
  context 'invalid :include' do
    ...
    context 'an include to an inexistent model' do
      it 'raises an exception' do
        ...
        expect do
          MyApp::SongSerializer.side_loads([MyApp::Song.first],
                                           RestPack::Serializer::Options.new(MyApp::SongSerializer,
                                                                             'include' => 'wrong'))
    ...
    context "an include to a model which has not been whitelisted with 'can_include'" do
      it 'raises an exception' do
        ...
        expect do
          MyApp::ArtistSerializer.side_loads([payment.artist],
                                             RestPack::Serializer::Options.new(MyApp::ArtistSerializer,
                                                                               'include' => 'payments'))
        end.to raise_error(exception, message)
      end
    end
  ...
  describe '#can_include' do
    ...
    it 'allows includes to be specified' do
      ...
      expect(CustomSerializer.can_includes).to eq(%i[model1 model2 model3])
    end
  end
  ...
  describe '#filterable_by' do
    ...
    context 'a model with a single :belongs_to relations' do
      it 'is filterable by primary key and foreign keys' do
        expect(MyApp::AlbumSerializer.filterable_by).to eq(%i[id artist_id year])
      end
    end
    context 'a model with multiple :belongs_to relations' do
      it 'is filterable by primary key and foreign keys' do
        expect(MyApp::SongSerializer.filterable_by).to eq(%i[id artist_id album_id title])
      end
    end
  end
end
